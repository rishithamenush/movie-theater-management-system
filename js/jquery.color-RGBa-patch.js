...
(function (jQuery) {
        ...
        if (fx.colorFunction == undefined || fx.state == 0) {
          ...
          if (fx.start == undefined) {
            ...
            if (fx.start[3] == undefined)
              // if alpha channel is not spotted
              fx.start[3] = 1; // assume it is fully opaque
            if (fx.start[3] == 0)
              // if alpha is present and fully transparent
          ...
          fx.colorFunction =
            fx.start[3] == 1 && fx.end[3] == 1 ? calcRGB : calcRGBa;
        }
  ...
  var calcRGB = function () {
    return (
      ...
      Math.max(
        Math.min(
          parseInt(this.pos * (this.end[0] - this.start[0]) + this.start[0]),
          255
        ),
        0
      ) +
      ...
      Math.max(
        Math.min(
          parseInt(this.pos * (this.end[1] - this.start[1]) + this.start[1]),
          255
        ),
        0
      ) +
      ...
      Math.max(
        Math.min(
          parseInt(this.pos * (this.end[2] - this.start[2]) + this.start[2]),
          255
        ),
        0
      ) +
  ...
  var calcRGBa = function () {
    return (
      ...
      Math.max(
        Math.min(
          parseFloat(this.pos * (this.end[3] - this.start[3]) + this.start[3]),
          1
        ),
        0
      ) +
